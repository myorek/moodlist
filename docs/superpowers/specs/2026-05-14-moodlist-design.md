...
- Free-form natural-language input via Alfred (`ml <query>`).
...
`ml top 80s metal` (typed into Alfred):
...
- `--fresh` flag (Alfred prefix `ml!`): bypass `lookup`, force a new agent
...
- The Alfred workflow has **one keyword** (`ml`) with two variants resolved
  by argument prefix:
  - `ml <query>` → `moodlist "<query>" --alfred-json`
  - `ml! <query>` → `moodlist "<query>" --alfred-json --fresh`
...
- A separate Alfred keyword `ml-reindex` runs `moodlist --reindex` and
...
| Library empty | Suggest `ml-reindex` in Alfred subtitle |
...
- **2026-05-14** — cache never expires; `--fresh` (`ml!` prefix) overrides.
